...
Os apps iOS e Android usam o mesmo mecanismo de leitura que o desktop, então abrem os mesmos formatos e lembram seu lugar da mesma forma. Eles foram criados para serem usados com VoiceOver no iOS e TalkBack no Android.

### Abrindo documentos

* Use o botão Open Book, ou abra um documento do app Files ou outro app e escolha Paperback.
* No Android, você pode ativar o navegador de arquivos integrado em Settings. Ele precisa da permissão All Files Access e abre arquivos grandes imediatamente em vez de copiar primeiro.
* Pressione longamente o botão Open Book para importar ou exportar os dados de um documento (`.paperback`), os mesmos arquivos que o app desktop usa.

### Leitura e escuta

Cada app tem duas formas de ler um documento. No modo texto, você lê o texto com seu leitor de tela. No modo leitura em voz alta, Paperback lê o texto para você com a voz que você escolher em Settings, e continua no fundo e na tela de bloqueio. Alterne entre eles no menu More Options.

Livros de áudio, como DAISY, M4B e livros em MP3, executam sua própria gravação.

### A barra de leitura

A barra na parte inferior da tela tem, da esquerda para a direita:

* A unidade de navegação, como parágrafo, título, página ou link. Deslize para cima ou para baixo nela para alterá-la.
* Botões anterior, reproduzir e próximo. Anterior e próximo movem pela unidade de navegação.
* A velocidade de fala. Deslize para cima ou para baixo nela para alterar a velocidade de leitura do Paperback.

Você também pode deslizar para cima ou para baixo no botão reproduzir para mover pela unidade de navegação, sem alcançar os botões anterior e próximo. Se for tudo o que você usar, a configuração Hide previous and next buttons os remove do seu leitor de tela. A configuração Swipe up moves forward escolhe para qual direção um deslize vai.

### Mais opções

O menu More Options é onde tudo o mais fica. Alguns itens funcionam um pouco diferente em cada app.

* **Switch to TTS Mode ou Switch to Text Mode:** alterna entre o modo leitura em voz alta e o modo texto, descrito acima. Android também tem um item Read Aloud que inicia e pausa a leitura em voz alta.
* **Table of Contents:** os capítulos do livro. Escolha um para ir direto para ele. No Android, entradas com capítulos embaixo delas podem ser expandidas e recolhidas, usando as ações do leitor de tela. No iOS, a lista inteira é mostrada de uma vez.
* **Elements:** uma lista de títulos ou links do documento. Alterne entre os dois com o seletor Type no iOS, ou as abas no Android, depois escolha um para ir para ele.
* **Find:** digite o que procurar e escolha se deseja diferenciar maiúsculas e minúsculas, corresponder apenas palavras inteiras ou usar uma expressão regular. No Android, uma barra com Find Previous e Find Next fica na parte inferior da tela até você fechá-la, e buscas anteriores estão em Search History. No iOS, os botões Find Previous e Find Next estão na tela Find, e Find também aparece como uma unidade de navegação na barra de leitura, para que você possa passar pelos resultados de lá.
* **Go To:** pule para uma linha, uma página ou uma porcentagem do documento. Escolha qual com o seletor Mode.
* **Recent Documents:** todos os documentos que você abriu, cada um marcado como atualmente aberto, fechado ou arquivo ausente. Cada um tem duas ações do leitor de tela: Remove o tira da lista, e Locate permite que você encontre um documento cujo arquivo se moveu. Clear Recent Documents esvazia a lista sem apagar documentos.
...
* **Sleep Timer:** para a leitura após 5, 10, 15, 30, 45 ou 60 minutos, ou um horário de sua escolha. Abra novamente enquanto estiver em execução para ver quanto tempo resta, ou para cancelar.
* **Help:** abre este readme.
* **Settings:**
    * **Text to speech:** a voz, velocidade de fala e tom, um botão Play Sample para ouvi-los e a pausa entre parágrafos. Android também permite que você escolha o mecanismo de fala. No iOS, este também é o lugar do dicionário de fala: regras que alteram a forma como as palavras são faladas, para todas as vozes ou apenas algumas.
    * **Readability:** tamanho do texto, espaçamento entre linhas, espaçamento entre parágrafos e alinhamento. iOS também tem aparência clara e escura, e texto de alto contraste.
    * **Behavior:** se deve reabrir seus documentos quando o app inicia, para qual direção um deslize no botão reproduzir se move e se deve ocultar os botões anterior e próximo. Android também tem o navegador de arquivos integrado aqui.

### Teclados e fones de ouvido

Com um teclado, os atalhos do desktop para abrir livros, documentos recentes, Find, Go To, a tabela de conteúdos, contagem de palavras, informações do documento, exportação e o temporizador de sono funcionam todos, usando `Cmd` em vez de `Ctrl` no iOS. O mesmo vale para as teclas de letra única para mover por título, página, link e o resto, e `Space` reproduz e pausa. No iOS, as teclas de letra única apenas alcançam o Paperback enquanto o Quick Nav de letra única do VoiceOver está desativado.

No Android, um botão de fone de ouvido reproduz e pausa com um pressionamento, avança com dois e volta com três.
...
A versão 1.0 é o primeiro lançamento em todas as cinco plataformas: Windows, macOS, Linux, iOS e Android, com os aplicativos iOS e Android na App Store e Google Play.

#### Adicionado

##### Geral
* Suporte a Linux, como um AppImage ou tar.gz, com integração de desktop para que documentos sejam abertos pelo seu gerenciador de arquivos.
* Marque o início de uma seleção com `Alt+F9`, copie tudo dali até onde você chegou com `Alt+F10`, e volte à marca com `Alt+Shift+F9`, para copiar um grande trecho de texto sem passar por ele com shift e setas. Todos os três estão em Tools > Select and copy.
* O atalho `=` agora anuncia a página bem como a porcentagem, por exemplo, "15%, página 30", e permanece como era para documentos sem números de página.
...
* Páginas de manual, tanto `man` quanto `mdoc` do BSD, compactadas ou não.
* Audiolivros MP3, divididos em capítulos quando o arquivo os possui.
* Arquivos Windows Write (`.wri`).
* Arquivos WinHelp (`.hlp`).
* Documentos Word 6 e Word 95.

##### OCR
* Páginas de PDF digitalizadas agora podem ser reconhecidas com o OCR integrado no Windows e macOS. Pressione `Enter` em uma página digitalizada para reconhecê-la, ou use Batch OCR (`Ctrl+Shift+O`) para um intervalo de páginas.

##### Navegação
* Fórmulas MathML em EPUB e HTML são renderizadas como AsciiMath usando MathCAT. Use `M` ou `Shift+M` para navegar pelas fórmulas, então `Enter` ou `Space` para abrir o MathML original na Visualização de Fórmula.
* Um botão Find All (Localizar Todos) na caixa de diálogo Localizar, listando cada linha com uma correspondência para que você possa pular direto para a que deseja.
* Visualizações de Tabelas, Listas e Páginas na lista de elementos (`F7`).
* Go to Line, Go to Page e Go to Percent agora aceitam `+n` e `-n` para se mover relativamente de onde você está.
* Livros EPUB, MOBI e CHM sem seus próprios títulos agora recebem navegação de títulos de seu índice.
* Livros KF8 (AZW3) agora suportam navegação por seção.
* Páginas EPUB que são apenas uma imagem agora mostram uma linha para ela, para que você possa descer nelas em vez de pular direto por elas.

##### Audiolivros
* Controles de velocidade de reprodução, de meia velocidade a três vezes mais rápido. Use `Ctrl+Shift+.` e `Ctrl+Shift+,`, ou o menu Ferramentas.
* Marcadores e notas em livros apenas com áudio agora lembram o tempo exato em que você os definiu.
* Próxima e posição anterior (`Alt+Left` e `Alt+Right`) agora funcionam em audiolivros.
* O progresso através de um audiolivro agora é medido por sua gravação, então Go to Percent e a barra de status correspondem ao quanto você realmente percorreu.

##### Documentos Recentes
* Um item Clear Recent Documents (Limpar Documentos Recentes) no submenu Documentos Recentes.
...
* PDFs que possuem estrutura de leitura mas não marcam nenhuma de suas imagens agora anunciam essas imagens, em vez de omiti-las inteiramente do livro.

##### Visualização na Web
* Qualquer documento agora pode ser aberto na visualização na web, não apenas EPUB, HTML e Markdown.
...
* pb agora diz qual arquivo ele não conseguiu ler e por quê.

#### Corrigido

##### Geral
* Corrigido um travamento ao fechar Paperback.
* Fechar Paperback agora oculta a janela imediatamente, em vez de deixá-la na tela enquanto salva.
* Abrir um documento não deixa mais Reabrir Último Fechado ativado quando não há nada para reabrir.
* Paperback não tenta mais abrir repetidamente documentos da sua lista recente que desapareceram, e limita quantos documentos recentes armazena.
* O arquivo antigo de configurações INI agora é excluído após ser movido para o novo formato.
...
##### Navegação
* `Alt+Left` agora volta para onde você saltou, em vez de ir para uma posição mais antiga.
* Sons de marcadores agora tocam apenas quando você se move sobre um marcador, não quando você chega à linha em que ele está.
* Fechar a tabela de conteúdos, a lista de elementos e os diálogos Ir para agora leva você direto à linha em que você chega, em vez de fazer você ouvir o leitor de tela ler a janela novamente.
...
* NVDA não corta mais o anúncio quando um documento não tem páginas.
* Pressionar OK na tabela de conteúdos sem se mover agora vai para a entrada que já estava selecionada.
* A tabela de conteúdos, a lista de elementos e a lista de marcadores não ficam mais lentas ou congelam em livros com milhares de entradas.
* As setas para cima e para baixo agora lembram sua coluna por documento, em vez de carregá-la quando você muda de abas.

##### Audiolivros
* A reprodução de áudio agora usa `Control+Space` no macOS, pois `Command+Space` pertence ao Spotlight.

##### Documentos PDF
* Corrigidos PDFs exportados do Apple Pages lidos como texto simples, sem os títulos e listas com que foram escritos.
* Corrigidos parágrafos e títulos PDF dividindo em cada linha, e palavras se dividindo em espaços.
* Corrigidos títulos PDF numerados se unindo em um único título.
* Corrigidos PDFs cuja árvore de estrutura não leva a nenhum texto abrindo vazios.
* Cabeçalhos e rodapés de página não são mais lidos em cada página de PDFs sem marcação.
* PDFs que marcam seus cabeçalhos e rodapés de página como texto ordinário não repetem mais o título e o número da página entre dois parágrafos em cada página.
* PDFs agora mostram seu título real, em vez do nome do arquivo.
* Linhas definidas em uma fonte monoespaçada, como código, não são mais unidas em parágrafos.
...
* Corrigidos livros MOBI perdendo sua lista de capítulos.
* Corrigido texto corrompido onde livros MOBI passam de um registro para o próximo.

##### Visualização na Web
* A visualização na web não carrega mais a totalidade de um livro enorme de uma vez.
* A visualização na web agora mostra documentos inteiros quando o leitor os mostra inteiros, em vez de apenas uma fatia deles.

##### Outros Formatos
* Livros FictionBook (.fb2) escritos em windows-1251, que é a maioria deles, agora abrem em vez de falhar ao ler.
* Livros FictionBook que usam um namespace ou uma entidade HTML que nunca declararam agora abrem, em vez de serem recusados como corrompidos.
* Livros em codificações herdadas agora abrem muito mais rápido.
* Corrigidos alguns arquivos de texto em chinês abrindo como texto corrompido.
* Arquivos OpenDocument protegidos por senha agora pedem sua senha, em vez de serem relatados como corrompidos.
* Arquivos PowerPoint herdados protegidos por senha agora abrem, e slides PowerPoint herdados não perdem mais seu texto.
* Arquivos de texto simples salvos com uma extensão `.rtf` agora abrem como texto, em vez de falhar com um erro.
* Palavras de controle RTF não aparecem mais como texto.

#### iOS e Android

Os aplicativos iOS e Android abrem todos os formatos que o desktop abre e incluem:

* Leitura em voz alta, com sua escolha de voz, velocidade e tom, um controle de velocidade de fala direto na barra de leitura e uma pausa opcional entre parágrafos.
* Reprodução de audiolivros DAISY, M4B e MP3, que continua funcionando em segundo plano e na tela de bloqueio.
...
* Um temporizador de sono, contagem de palavras e exportação de documentos, mais um dicionário de fala no iOS.
* Opções de tamanho e espaçamento de texto, mais texto de alto contraste no iOS.
...
* Audiobooks agora nomeiam o arquivo conforme você avança por seção.
* Audiobooks agora informam seu comprimento real, em vez de alegar que cada arquivo neles dura 24 horas.
* Fechar a Web View com Escape não lança mais um alerta de debug após você ter seguido um link dentro dela.
* Copiar após Select All agora fornece o documento inteiro, em vez de apenas a parte dele carregada no momento.
* Find agora vai direto à linha encontrada, em vez de fazer você ouvir o leitor de tela ler a janela novamente conforme o foco retorna ao livro.
* Corrigidos EPUBs que carregam um bloco ZIP64 isolado recusando abrir com "Invalid local file header".
* Corrigidos documentos longos retrocedendo para seu início enquanto um leitor de tela lia continuamente por eles.
* Links na WebView agora levam você à seção para a qual apontam, em vez de falhar com "File not found".
...
* A aba General do diálogo Settings agora tabulação por suas opções na ordem em que aparecem na tela, com o canal de atualização diretamente após a opção de verificar atualizações.
* Windows agora sempre mostrará "Paperback" no menu Open With, em vez da tagline completa do programa.
* Word Count e Document Info agora mostram quantos arquivos um audiobook contém e quanto tempo dura no total.

### Versão 0.9.1
* Sons de marcador e anotação agora tocam no macOS.
* Livros DAISY agora tocam seu áudio no macOS, em vez de abrir e rastrear sua linha do tempo em silêncio.
* Corrigidas aspas curvas, travessões e caracteres similares desaparecendo de documentos RTF, juntando as palavras ao redor conforme desaparecem.
* Corrigidas imagens RTF vazando seus dados brutos no documento como texto corrompido.
* Corrigido o submenu Recent Documents mantendo entradas obsoletas até que algo aconteça para reconstruí-lo.
* Aceleradores de teclado estão de volta em cada tradução, então os menus do russo têm acesso de teclado novamente.
...
* Formas plurais agora são traduzidas, então mensagens que contam coisas leem adequadamente em idiomas que precisam de mais de uma forma.
* Selecionar ncc.html de um livro DAISY agora abre o audiobook completo em vez de apenas seu texto.
* Os nomes de ação do diálogo Customize Keyboard Shortcuts agora podem ser traduzidos.
...
* O texto do documento agora é paginado, o que significa que você pode carregar livros com dezenas de milhões de palavras em apenas alguns segundos. Por favor, informe qualquer comportamento estranho encontrado com isso.

##### Suporte de Plataforma
* Suporte para ARM64 no Windows!
* Suporte nativo para macOS!
* Um botão de alternância de tela inteira.

##### Diálogo Todos os Documentos
* Um botão localizar para localizar livros que apenas mudaram seu caminho.
...
* Um item de menu de quebra de linha e uma tecla de atalho subsequente.
* Uma alternância para determinar como você deseja que as tabelas sejam exibidas, e unificou como as tabelas são exibidas em todos os documentos.
...
* Se uma seleção estiver ativa quando você abrir o diálogo de contagem de palavras, agora será mostrado quantas palavras você selecionou.
...
##### Exportar
* Expandiu o item de menu de exportação para permitir exportação para HTML e Markdown, além de texto simples.
...
##### Visualizador da Web
* A visualização da web agora é aberta em sua posição de leitura atual.
...
* Atalhos de teclado e itens de menu para reproduzir/pausar narração, procurar para frente e para trás e ajustar a quantidade de busca.
* Opções para sincronizar o cursor de leitura com a reprodução de áudio, definir a quantidade de busca de áudio e escolher se a busca além do final de um capítulo continua no próximo.
...
* "Reabrir último fechado" tentando reabrir o readme integrado.
* Sua guia selecionada não estava sendo corretamente focada após reiniciar o Paperback.
* Tratamento do Paperback de arquivos em unidades de rede do Windows: pressionar mostrar arquivo na pasta agora corretamente foca o arquivo no armazenamento de rede, e os caminhos não contêm mais caracteres estranhos.
...
* Abrir o readme agora respeitará o seu idioma selecionado.
* A interface do usuário do Paperback agora será dimensionada corretamente em exibições com alto DPI.
* O menu agora é atualizado corretamente e o foco se move para o controle de texto ao abrir a ajuda no Paperback.
...
##### Marcadores
* Os sons de marcador/nota agora devem ser reproduzidos exclusivamente quando você navega sobre uma palavra contendo um.

##### Legibilidade
* Aplicar quebra de linha o levando para o início do seu documento.

##### Visualização da Web
* O diálogo de visualização da web não sendo redimensionável e aparecendo com um tamanho inicial muito pequeno.
* As imagens agora devem ser exibidas corretamente na visualização da web incorporada.

##### Atualizador
* O atualizador agora mostra corretamente o conteúdo das tags de código markdown nas notas de versão.
...
##### Documentos PDF
* O Paperback agora retorna para extração de texto simples para PDFs falsamente marcados.
...
* O texto alternativo para imagens incorporadas agora deve ser exibido corretamente.
...
* Corrigido o menu de contexto do leitor não aparecer ao clicar com o botão direito ou pressionar a tecla Aplicações.
* Corrigido o documento errado às vezes sendo focado ao abrir documentos da linha de comando.
* PDFs somente com imagens são novamente detectados e alertam você sobre sua existência.
* Agora é possível navegar através de imagens e figuras com g/shift+g e f/shift+f, respectivamente.
* O Paperback agora respeitará a configuração de modo escuro da sua aplicação.
* Removido suporte DAISY XML, pois não é mais necessário.
* Voltado para a navegação de primeira letra nativa Win32 na árvore do índice.
* O diálogo de erro de carregamento agora mostra mensagens de erro mais detalhadas.
* A visualização da web agora será aberta muito mais rápida e suavemente.
...
* Corrigidos alguns bugs do analisador RTF.
* Corrigidos caminhos de arquivo contendo caracteres não-ASCII (como š, č, ć, ž bósnios) se tornando corrompidos ao abrir um arquivo através de uma segunda instância do Paperback.
...
* Adicionador um atualizador automático que agora substituirá sua versão instalada do Paperback em vez de apenas baixar a nova versão!
* Adicionado feedback sonoro opcional ao alcançar um marcador ou uma nota, obrigado Andre Louis pelos sons!
* Adicionado suporte para documentos RTF!
* Adicionado suporte para documentos DAISY XML.
* Adicionado suporte para arquivos de Texto de Documento Aberto Flat!
* Adicionado suporte para apresentações de Documento Aberto Flat!
...
* Corrigido documentos Markdown exibindo texto bruto em vez de HTML renderizado na Exibição da Web.
...
* Diálogo de opções dividido em abas para facilitar o uso e a navegação.
...
### Versão 0.7.0
* Adicionado suporte a tabelas para documentos baseados em HTML e XHTML! Navegue entre tabelas usando T e Shift+T, e pressione Enter para visualizar uma em um webview.
* Adicionado um recurso básico de renderização da web! Pressione Ctrl+Shift+V para abrir a seção atual do seu documento em um renderizador baseado na web, útil para conteúdo como formatação complexa ou exemplos de código.
* Adicionada tradução russa, obrigado Ruslan Gulmagomedov!
* Adicionado botão Limpar Tudo ao diálogo Todos os Documentos.
...
* Corrigido diálogo de localização não se ocultando adequadamente ao usar botões próximo/anterior.
* Corrigido TOC do epub ocasionalmente o levando para o item errado.
* Corrigidos vários problemas de tratamento de espaço em branco em XML, HTML e tags pre.
* Corrigido erro de off-by-one na navegação de links.
* Corrigidos alguns livros com espaço em branco à direita em suas linhas.
...
* Melhorado tratamento de listas em vários formatos de documento.
* Melhorado fluxo de tradução para contribuidores.
...
* Adicionado um recurso bem básico de ir para posição anterior/próxima. Se você pressionar enter em um link interno e ele mover seu cursor, essa posição agora será lembrada e poderá ser navegada com as setas alt+left/right.
...
* Corrigida análise de TOCs do Epub contendo caminhos relativos.
* Corrigidos alguns documentos epub não exibindo título ou autor.
* Corrigidos títulos de alguns capítulos de epub não aparecendo corretamente no diálogo TOC.
* Corrigido você não conseguindo usar a barra de espaço para ativar os botões OK/cancelar no diálogo TOC.
* Melhorado o tratamento de títulos em documentos Word.
* Você agora receberá feedback falado se a lista de documentos recentes estiver vazia quando você tentar abrir o diálogo.

### Versão 0.6.0
* Uma nova opção para mostrar o menu de navegação de forma muito mais compacta foi adicionada ao diálogo de opções, marcada por padrão.
...
* Os marcadores agora podem marcar uma linha inteira ou marcar apenas algum texto especificado. Se você não tiver uma seleção ativa ao colocar um marcador, o comportamento é como pré-0.6, e ele marcará a linha inteira. No entanto, se você selecionar algum texto, apenas esse texto será incluído no marcador.
* Os marcadores agora podem ter notas de texto opcionais anexadas a eles! Navegue entre marcadores contendo notas com N e Shift+N, ou abra o diálogo de marcadores com todos os marcadores, apenas notas ou apenas não-notas selecionadas com teclas de atalho específicas.
* Os marcadores no diálogo de marcadores não terão mais um prefixo "marcador x" incômodo.
* Livros Epub contendo conteúdo HTML pretendendo ser XML agora serão tratados corretamente.
* Corrigido o carregamento de grandes documentos Markdown.
* Corrigido o pressionamento da barra de espaço na visualização em árvore do sumário ativando o botão OK.
...
* Se carregando um livro com um parâmetro de linha de comando enquanto uma instância existente do Paperback está em execução, você não receberá mais um erro se o carregamento do seu documento levar mais de 5 segundos.
* Se executando o Paperback como administrador, a configuração agora será carregada e salva corretamente.
* Agora é possível excluir um marcador diretamente do diálogo de marcadores.
* Agora é possível importar e exportar seus marcadores e posição de leitura para um documento específico. O arquivo gerado é nomeado após o arquivo com uma extensão .paperback. Se tal arquivo for encontrado no mesmo diretório que um arquivo ao carregá-lo, ele será carregado automaticamente. Caso contrário, você pode importá-los manualmente usando um item no menu de ferramentas.
...
* O conteúdo Markdown agora é pré-processado para estar em conformidade com CommonMark antes da renderização.
...
* O Paperback agora pode opcionalmente minimizar para a sua bandeja do sistema! Esta opção está desativada por padrão, mas ativá-la fará com que a opção minimizar no menu do sistema coloque o Paperback em sua bandeja, podendo ser restaurado clicando no ícone gerado.
* O Paperback agora é totalmente traduzível! A lista de idiomas que suporta é atualmente bastante pequena, mas está crescendo constantemente!
...
* A lista de documentos recentes foi dramaticamente expandida! Em vez de simplesmente mostrar os últimos 10 documentos que você abriu, agora mostrará um número personalizável, com os demais documentos que você já abriu sendo acessíveis por um pequeno diálogo.
...
* Corrigida a orientação do controle deslizante de ir para percentual.
* Corrigido o índice em livros Epub com caminhos de arquivo codificados em URL e/ou IDs de fragmento.
* Corrigido espaçamento em branco sendo removido de títulos XHTML de maneiras estranhas.
* Corrigido o tratamento de espaçamento em branco dentro de tags pre aninhadas em documentos HTML.
* Documentos HTML e Markdown agora suportam o recurso de índice! Quando você carrega um documento HTML/Markdown, Paperback criará seu próprio índice a partir da estrutura dos títulos do seu documento, e o mostrará a você na caixa de diálogo `ctrl+t`.
* Documentos HTML agora terão o título conforme definido na tag title, se existir. Caso contrário, continuarão a usar o nome do arquivo sem a extensão.
* Mudado de UniversalSpeech para usar uma região ativa para relatar fala. Isso significa que nenhuma DLL de leitor de tela é mais enviada junto com o programa, e mais leitores de tela agora serão suportados, como o Microsoft Narrator.
* Mudadas bibliotecas zip para permitir a abertura de uma gama mais ampla de livros epub.
...
* A caixa de diálogo ir para percentual agora inclui um campo de texto que permite que você insira manualmente um percentual para ir.
...
### Versão 0.4.1
* Adicionado um ícone opcional do menu Iniciar ao instalador.
...
* Suporte adicionado para marcadores! Você pode ter quantos marcadores desejar em quantos documentos desejar. Você pode pular para frente e para trás através deles com `b` e `shift+b`, definir um com `control+shift+b`, e abrir uma caixa de diálogo para pular para um marcador específico com `control+b`.
* Adicionado um instalador junto com o arquivo zip portátil! O instalador instalará Paperback em seu diretório Program Files e configurará automaticamente as associações de arquivo para você.
* Arquivos de texto com BOMs agora devem ser decodificados corretamente, e o BOM não será mais exibido no início do texto.
* Adicionadas muito mais informações à barra de status. Agora mostrará sua linha atual, caractere e percentual de leitura.
* Comentários HTML, bem como o conteúdo de tags script e style, não serão mais exibidos na saída de texto.
* Se passar um caminho relativo para Paperback na linha de comando, agora ele será resolvido corretamente.
* O movimento de percentual agora é tratado por sua própria caixa de diálogo baseada em controle deslizante, acessível com `control+shift+g`.
* Documentos sem títulos ou autores conhecidos agora sempre terão um padrão.
* A lógica de salvamento de posição agora é muito mais inteligente e deve apenas gravar no disco quando absolutamente necessário.
* O documento que você tinha em foco quando fechou Paperback agora é lembrado entre reinicializações do aplicativo.
* A entrada nas caixas de diálogo ir para linha e ir para página agora deve ser higienizada mais rigorosamente.
* Corrigida a navegação do índice em livros epub 3 com caminhos relativos em seus manifestos.

### Versão 0.3.0
* Corrigido o índice em livros epub com manifestos codificados em URL.
* Corrigida a navegação de títulos em documentos HTML contendo caracteres Unicode multibyte.
...
* Adicionada uma caixa de seleção na caixa de diálogo de opções para ativar ou desativar quebra de linha!
* Agora é possível fazer uma doação para o desenvolvimento do Paperback, seja através do novo item de doação no menu de ajuda ou através do link patrocinar este projeto na parte inferior da página principal do repositório GitHub.
...
* Mudadas bibliotecas PDF para a usada no Chromium, resultando em análise de PDF muito mais confiável em geral.
...
* Permitida a tabulação do conteúdo do documento para sua lista de documentos abertos.
...
* Uma mensagem agora é reproduzida se o documento não suportar um índice ou seções, em vez de os itens de menu serem desativados.
* Adicionado menu de documentos recentes! Atualmente armazena seus últimos 10 documentos abertos, e pressionar enter em um deles abrirá para leitura.
* Completamente reescrito o diálogo Localizar, tornando-o muito mais simples de usar, além de adicionar um histórico de suas últimas 25 buscas e suporte a expressões regulares!
...
* Adicionado `shift+f1` para abrir o arquivo leia-me diretamente no Paperback.
