...
O Paperback funciona bem com todos os principais leitores de tela. Porém, há um problema conhecido para usuários do JAWS.

### JAWS e Displays Braille

Se você usar JAWS com um display Braille, pode descobrir que parágrafos longos são truncados ao navegar para frente com as teclas de navegação do seu display. O comando de leitura do parágrafo atual também é afetado. Este é um bug no tratamento do controle de texto RICHEDIT50W pelo JAWS, não algo no próprio Paperback, e um que demorou bastante para que uma correção viesse à tona, dada a falta de entusiasmo da Vispero em responder a problemas de software de código aberto.

A solução alternativa, eventualmente descoberta através do grupo de discussão do JAWS após meses de espera, é editar `paperback.jcf` e definir "Braille Presentation and Panning" como "Always use DOM if available". Você também vai querer ativar "Pan Text by Paragraph", caso contrário seu display permanecerá no parágrafo ativo em vez de avançar. Com ambas as configurações em vigor, a navegação deve funcionar corretamente.

### JAWS e as mensagens do Paperback

O Paperback diz coisas como "No pages." ou "This document has no audio." como notificações de acessibilidade, o que permite que um leitor de tela as fale sobre qualquer outra coisa que esteja dizendo. O JAWS só age sobre elas quando "Enable accessible notification events" está ativado para a aplicação, e em algumas máquinas não está.

Se o JAWS não disser nada quando você pressionar uma tecla que deveria informar algo, abra o Settings Center com o Paperback em primeiro plano (`Insert+6`), procure por "notification", e marque "Enable accessible notification events". Isso escreve a configuração em `paperback.jcf`, então se aplica apenas ao Paperback.
...
##### Geral
* Suporte a Linux, como AppImage ou tar.gz, com integração com o gerenciador de arquivos para que documentos abram a partir dele.
* Marque o início de uma seleção com `Alt+F9`, copie tudo dali até onde você chegou com `Alt+F10`, e volte à marca com `Alt+Shift+F9`, para copiar um longo trecho de texto sem usar shift-arrow. Os três estão em Tools > Select and copy.
* O atalho `=` agora anuncia a página além da porcentagem, por exemplo "15%, página 30", e permanece como era para documentos sem números de página.
...
* Páginas de manual, tanto `man` quanto BSD `mdoc`, compactadas ou não.
...
##### OCR
* Páginas de PDF digitalizadas agora podem ser reconhecidas com o OCR integrado ao Windows e macOS. Pressione `Enter` em uma página digitalizada para reconhecê-la, ou use Batch OCR (`Ctrl+Shift+O`) para um intervalo de páginas.
...
* Um botão Find All no diálogo Find, listando cada linha com uma correspondência para que você possa pular direto para a que deseja.
* Exibições de Tables, Lists e Pages na lista de elementos (`F7`).
* Go to Line, Go to Page e Go to Percent agora aceitam `+n` e `-n` para se mover relativamente a onde você está.
* Livros EPUB, MOBI e CHM sem títulos próprios agora obtêm navegação por títulos de seu sumário.
...
* Marcadores e notas em livros somente áudio agora lembram o tempo exato em que você os definiu.
...
* PDFs que carregam estrutura de leitura mas não marcam nenhuma de suas imagens agora anunciam essas imagens, em vez de deixá-las de fora do livro inteiramente.
...
* O Paperback não tenta mais reabrir documentos da sua lista recente que desapareceram, e limita quantos documentos recentes armazena.
* O arquivo de configurações INI antigo agora é excluído após ser migrado para o novo formato.
...
* A quebra de linha agora é aplicada imediatamente em documentos grandes, em vez de recarregar tudo.

##### Navegação
* `Alt+Left` agora volta para onde você pulou, em vez de ir para uma posição mais antiga.
* Sons de marcadores agora só tocam quando você passa sobre um marcador, não quando você chega na linha em que ele está.
* Fechar a tabela de conteúdos, a lista de elementos e os diálogos Ir para agora leva você direto para a linha em que você chega, em vez de fazer você ouvir o leitor de tela ler a janela novamente.
...
* A tabela de conteúdos, a lista de elementos e a lista de marcadores não travam mais ou congelam em livros com milhares de entradas.
* Os botões Cima e Baixo agora lembram sua coluna por documento, em vez de carregá-la quando você muda de abas.

##### Audiolivros
* A reprodução de áudio agora usa `Control+Space` em macOS, já que `Command+Space` pertence ao Spotlight.

##### Documentos PDF
* Corrigido PDFs exportados do Apple Pages sendo lidos como texto simples, sem os títulos e listas com os quais foram escritos.
* Corrigido PDFs com parágrafos e títulos divididos a cada linha, e palavras separadas em espaços.
...
* Cabeçalhos e rodapés de página não são mais lidos em cada página de PDFs sem tags.
* PDFs que marcam seus cabeçalhos e rodapés de página como texto ordinário não repetem mais o título e o número da página entre dois parágrafos a cada página.
...
* Corrigido texto distorcido onde livros MOBI cruzam de um registro para o próximo.

##### Visualização da Web
* A visualização da web não carrega mais o todo de um livro gigante de uma vez.
...
##### Outros Formatos
* Livros FictionBook (.fb2) escritos em windows-1251, que é a maioria deles, agora abrem em vez de falharem ao ler.
...
* Livros em codificações antigas agora abrem muito mais rápido.
* Corrigido alguns arquivos de texto em chinês abrindo como texto distorcido.
* Arquivos OpenDocument protegidos por senha agora pedem sua senha, em vez de serem reportados como corrompidos.
* Arquivos PowerPoint antigos protegidos por senha agora abrem, e slides PowerPoint antigos não perdem mais seu texto.
* Arquivos de texto simples salvos com uma extensão `.rtf` agora abrem como texto, em vez de falharem com um erro.
* Palavras-chave de controle RTF não aparecem mais como texto.

#### iOS e Android

Os aplicativos iOS e Android abrem todos os formatos que a versão de desktop faz, e incluem:
...
* Navegação por títulos, páginas, links, tabelas, listas e muito mais na barra de leitura, além da tabela de conteúdos e Localizar.
* Um temporizador de sono, contagem de palavras e exportação de documentos, além de um dicionário de fala no iOS.
* Opções de tamanho de texto, espaçamento e texto com alto contraste.
* Atalhos de teclado que correspondem à versão de desktop.

### Versão 0.9.2
* Audiolivros não fazem mais o seu leitor de tela ler uma série de espaços quando você enfoca o campo de texto.
...
* Fechar a Visualização da Web com Escape não levanta mais um alerta de depuração após você ter seguido um link dentro dele.
* Copiar após Selecionar Tudo agora fornece todo o documento, em vez de apenas a parte dele carregada atualmente.
* Localizar agora vai direto para a linha que encontrou, em vez de fazer você ouvir o leitor de tela ler a janela novamente conforme o foco retorna ao livro.
* Corrigido EPUBs que carregam um bloco ZIP64 perdido recusando abrir com "Cabeçalho de arquivo local inválido".
* Corrigido documentos longos voltando para o início enquanto um leitor de tela lia continuamente por eles.
* Links na Visualização da Web agora levam você para a seção para a qual apontam, em vez de falharem com "Arquivo não encontrado".
* O anúncio automático "Documento recarregado" não corta mais seu leitor de tela no meio de uma frase, em vez disso esperando que ele termine o que estava dizendo.
* A aba Geral do diálogo Configurações agora passa pelas suas opções na ordem em que aparecem na tela, com o canal de atualização diretamente após a opção verificar atualizações.
...
### Versão 0.9.1
* Sons de marcadores e anotações agora tocam no macOS.
...
* Os aceleradores de teclado estão de volta em todas as traduções, então os menus do russo têm acesso ao teclado novamente.
...
* Options foi renomeado para Settings, acompanhando os aplicativos móveis e, no macOS, a convenção da plataforma.
* Paperback agora lembra a posição, tamanho e estado maximizado de sua janela entre as execuções.
...
* Um alternador de tela inteira.

##### Diálogo Todos os Documentos
* Um botão de localizar para localizar livros desaparecidos que acabaram de mudar de caminho.
...
* Um alternador para determinar como você deseja que as tabelas sejam exibidas e uma forma unificada de como as tabelas são exibidas em documentos.
...
* O atalho de teclado de igualdade para anunciar sua porcentagem atual por meio de um documento.
...
##### Atualizador
* Um botão de cancelamento para o diálogo de atualização em progresso.
...
##### Audiolivros
* A capacidade de reproduzir audiolivros, atualmente suportando DAISY de áudio (incluindo DAISY de áudio + texto) e zips de arquivos de áudio.
...
* Opções para sincronizar o cursor de leitura com a reprodução de áudio, definir a quantidade de busca de áudio e escolher se buscar além do final de um capítulo continua no próximo.
...
* Sua aba selecionada não recebendo o foco adequado após reiniciar o Paperback.
* O tratamento do Paperback para arquivos em unidades de rede do Windows: pressionar mostrar arquivo na pasta agora foca adequadamente o arquivo no armazenamento de rede, e os caminhos não contêm mais caracteres estranhos.
* Arquivos `.paperback` não serão mais carregados forcadamente na restauração de documentos; em vez disso, você será solicitado para confirmação quando um for encontrado.
...
* Reduzido uso de memória em documentos grandes reduzindo pela metade o tamanho das tabelas de índice interno por caractere.
...
* Readme.html não será mais adicionado à sua lista de todos os documentos ao ser aberto via `Shift+F1`.
...
##### Marcadores
* Os sons de marcador/nota agora devem ser reproduzidos adequadamente exclusivamente quando você navega sobre uma palavra contendo um.
...
* Imagens agora devem ser exibidas corretamente no webview incorporado.

##### Atualizador
* O atualizador agora exibe corretamente o conteúdo das tags de código markdown nas notas de lançamento.

##### Livros DAISY
* Livros DAISY exibindo informações incorretas na barra de status.
...
##### Documentos Word
* Documentos Word com nomes de estilos específicos de localidade não renderizando seus títulos corretamente.
...
* Corrigido o menu de contexto do leitor não aparecendo ao clicar com o botão direito ou pressionar a tecla Applications.
...
* Mudado de volta para a navegação por primeira letra Win32 nativa na árvore de conteúdo.
* O diálogo de erro ao carregar agora exibe mensagens de erro mais detalhadas.
* O webview agora abrirá muito mais rápido e suavemente.

### Versão 0.8.2
* Adicionado suporte de página para documentos RTF!
* Corrigido um bug onde abrir o webview em epubs contendo links externos os ativaria automaticamente.
...
* Documentos PDF agora possuem suporte básico de navegação de links e títulos!
* Abas RTF e alimentações de linha agora são renderizadas exatamente como aparecem no documento.
* Mudado de volta para a biblioteca pdfium confiável e comprovada para análise de PDFs, tornando a renderização de PDF muito mais confiável novamente.

### Versão 0.8.1
* Adicionado `Ctrl+Shift+T` para reabrir o último documento fechado.
* O diálogo Todos os Documentos agora suporta a seleção de vários documentos para abrir de uma vez.
* Corrigido alguns bugs com o analisador RTF.
* Corrigido caminhos de arquivo contendo caracteres não-ASCII (como š, č, ć, ž bósnios) se corrompendo ao abrir um arquivo via uma segunda instância do Paperback.
...
* Adicionado feedback sonoro opcional ao alcançar um marcador ou uma anotação, obrigado Andre Louis pelos sons!
...
* Qualquer movimento de mais de 300 caracteres agora adicionará automaticamente ao seu histórico de navegação.
* Corrigida a restauração da janela do Paperback a partir da bandeja do sistema.
* Corrigido o problema de documentos Markdown exibirem texto bruto em vez de HTML renderizado no Web View.
* Corrigidas tabelas não renderizadas adequadamente em arquivos Markdown.
* PDFs somente com imagem agora o alertarão sobre sua existência ao tentar carregar um.
* Informações de versão adequadamente incorporadas ao executável do Paperback.
* Divisão do diálogo de opções em abas para facilitar o uso e a navegação.
...
* O verificador de atualizações agora exibe notas de versão quando uma nova versão está disponível.
...
* Corrigido o diálogo de localização não sendo ocultado adequadamente ao usar os botões próximo/anterior.
* Corrigido o TOC do epub ocasionalmente o redirecionando para o item errado.
...
* Adicionado um recurso muito básico de ir para a posição anterior/próxima. Se você pressionar enter em um link interno e isso mover seu cursor, essa posição agora será lembrada e poderá ser navegada com as setas `Alt+Left`/`Alt+Right`.
* Adicionada uma lista de elementos! Atualmente, ela apenas mostra uma árvore de todos os títulos no seu documento ou uma lista de links, mas há planos para expandi-la no futuro.
...
* Corrigidos alguns documentos epub não exibindo um título ou autor.
* Corrigidos os títulos de alguns capítulos epub não aparecendo adequadamente no diálogo TOC.
* Corrigido o problema de você não conseguir usar a barra de espaço para ativar os botões OK/cancelar no diálogo TOC.
* Melhorado o tratamento de títulos em documentos Word.
* Você receberá feedback falado se a lista de documentos recentes estiver vazia ao tentar abrir o diálogo.
...
* Adicionado um recurso básico de temporizador de suspensão, acessível com `Ctrl+Shift+S`.
...
* Marcadores agora podem ser feitos para marcar uma linha inteira, ou para marcar apenas algum texto especificado. Se você não tiver nenhuma seleção ativa ao colocar um marcador, o comportamento é como pré-0.6, e marcará a linha inteira. No entanto, se você selecionar algum texto, apenas esse texto será incluído no marcador.
...
* Corrigido o pressionamento de espaço na visualização em árvore do índice ativando o botão OK.
...
* Se carregar um livro com um parâmetro de linha de comando enquanto uma instância existente do Paperback estiver em execução, você não receberá mais um erro se o carregamento do documento levar mais de 5 segundos.
...
* Delete no teclado numérico agora funciona para remover documentos da barra de abas além do delete normal.
* Paperback agora pode opcionalmente minimizar para sua bandeja do sistema! Esta opção está desativada por padrão, mas ativá-la fará com que a opção minimizar no menu do sistema coloque o Paperback em sua bandeja, podendo ser restaurado clicando no ícone gerado.
...
* Várias pequenas melhorias nos analisadores em toda a plataforma, incluindo colocar uma linha em branco entre slides em apresentações PPTX, corrigir o tratamento de quebra de linha dentro de parágrafos em documentos Word e adicionar pontos de marcação a itens de lista.

### Versão 0.5.0
* Adicionado suporte para documentos do Microsoft Word!
* Adicionado suporte para apresentações do PowerPoint!
* Corrigido certos itens de menu não serem desativados sem documentos abertos.
* Corrigida a orientação do controle deslizante de percentual de navegação.
* Corrigido o índice em livros Epub com caminhos de arquivo codificados em URL e/ou IDs de fragmento.
* Corrigido espaçamento em branco sendo removido de títulos XHTML de formas estranhas.
* Corrigido o tratamento de espaçamento em branco dentro de tags pre aninhadas em documentos HTML.
* Documentos HTML e Markdown agora suportam o recurso de índice! Quando você carrega um documento HTML/Markdown, o Paperback construirá seu próprio índice a partir da estrutura dos títulos no seu documento, e o exibirá a você na caixa de diálogo `ctrl+t`.
...
* Mudado de UniversalSpeech para usar uma região ativa para relatar fala. Isso significa que nenhuma DLL de leitor de tela é mais fornecida junto com o programa, e mais leitores de tela agora serão suportados, como o Microsoft Narrator.
* Bibliotecas zip alteradas para permitir abrir uma variedade maior de livros epub.
* A caixa de diálogo perguntando se você quer abrir seu documento como texto simples foi completamente refeita, e agora permite que você abra seu documento como texto simples, HTML ou Markdown.
* A caixa de diálogo de navegação por percentual agora inclui um campo de texto permitindo que você insira manualmente um percentual para pular para.
* O analisador HTML agora reconhecerá dd, dt e dl como elementos de lista.
* O índice em livros Epub será preservado exatamente mais uma vez.
...
* Documentos CHM agora devem exibir seu título conforme definido no arquivo de metadados.

### Versão 0.4.0
* Adicionado suporte para arquivos CHM!
* Adicionado suporte para marcadores! Você pode ter quantos marcadores quiser em quantos documentos quiser. Você pode navegar para frente e para trás entre eles com `b` e `shift+b`, definir um com `control+shift+b`, e abrir uma caixa de diálogo para pular para um marcador específico com `control+b`.
...
* Comentários HTML, assim como o conteúdo das tags script e style, não serão mais exibidos na saída de texto.
* Se passar um caminho relativo para o Paperback na linha de comando, agora o resolverá corretamente.
* O movimento por percentual agora é controlado por sua própria caixa de diálogo baseada em controle deslizante, acessível com `control+shift+g`.
...
* O documento em que você estava focado quando fechou o Paperback agora é lembrado em reinicializações do aplicativo.
* A entrada nas caixas de diálogo de ir para linha e ir para página agora deve ser sanitizada mais rigorosamente.
...
* Adicionada uma caixa de seleção na caixa de diálogo de opções para ativar ou desativar a quebra de linha!
* Agora é possível dotar o desenvolvimento do Paperback, seja através do novo item de doação no menu de ajuda ou através do link patrocinar este projeto na parte inferior da página principal do repositório do GitHub.
...
* Bibliotecas PDF alteradas para a usada no Chromium, levando a uma análise de PDF muito mais confiável em geral.
* Agora você pode ter apenas uma instância do Paperback executando por vez. Executar `paperback.exe` com um nome de arquivo enquanto já está em execução abrirá esse documento na instância já em execução.
* Você agora pode pressionar delete em um documento no controle de abas para fechá-lo.

### Versão 0.2.1
* Adicionado o número total de páginas ao rótulo de página na caixa de diálogo de ir para página.
* Permitir tabulação do conteúdo do documento para sua lista de documentos abertos.
* Corrigidos os atalhos de títulos às vezes abrindo documentos recentes se você tivesse o suficiente deles.
...
* Adicionadas teclas de atalho para navegar por títulos em conteúdo HTML, incluindo livros epub e documentos markdown. Essas teclas de atalho foram projetadas para funcionar de forma semelhante a um leitor de tela.
...
* Uma mensagem agora é reproduzida se o documento não suportar um sumário ou seções, em vez de os itens de menu serem desativados.
...
* Reescrita completa do diálogo Localizar, tornando-o muito mais simples de usar, enquanto também adicionado um histórico de suas últimas 25 buscas e suporte a expressões regulares!
