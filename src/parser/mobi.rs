...
impl HuffmanDecoder {
	...
	fn decode(&mut self, data: &[u8]) -> Result<Vec<u8>> {
		...
		loop {
			...
			if current.n <= 0 {
				if current.bits_left < 32 {
					for _ in 0..current.bits_left / 8 {
						if current.pos < current.data.len() {
							current.x = (current.x << 8) | u64::from(current.data[current.pos]);
							current.pos += 1;
						}
					}
					let tail = current.bits_left % 8;
					if tail > 0 {
						current.x <<= tail;
					}
				} else if current.pos + 4 <= current.data.len() {
					let v = u32::from_be_bytes([
						current.data[current.pos],
						current.data[current.pos + 1],
						current.data[current.pos + 2],
						current.data[current.pos + 3],
					]);
			...
			if !term {
				code_len += self.min_codes[code_len..].iter().position(|&mc| code >= mc).unwrap_or(0);
