...
long document_manager::find_text(const wxString& query, long start_pos, bool forward, bool match_case, bool match_whole_word) const {
	wxTextCtrl* text_ctrl = get_active_text_ctrl();
	if (!text_ctrl) return wxNOT_FOUND;
	const wxString& full_text = text_ctrl->GetValue();
	return ::find_text(full_text, query, start_pos, forward, match_case, match_whole_word);
}
