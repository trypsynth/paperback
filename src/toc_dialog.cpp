...
toc_dialog::toc_dialog(wxWindow* parent, const document* doc)
	...
	for (int id : { wxID_OK, wxID_CANCEL }) button_sizer->AddButton(new wxButton(this, id));
...
void toc_dialog::on_ok(wxCommandEvent& event) {
	if (selected_offset >= 0)
		...
		wxMessageBox("Please select a section from the table of contents.", "No Selection", wxOK | wxICON_INFORMATION,
		             this);
}
