...
impl Parser for EpubParser {
	...
	fn parse(&self, context: &ParserContext) -> Result<Document> {
		...
		let opf_doc = XmlDocument::parse_with_options(&opf_content, ParsingOptions { allow_dtd: true, ..ParsingOptions::default() }).context("Failed to parse OPF document")?;
...
fn find_container_path<R: Read + Seek>(archive: &mut ZipArchive<R>) -> Result<String> {
	...
	let doc = XmlDocument::parse_with_options(&container_xml, ParsingOptions { allow_dtd: true, ..ParsingOptions::default() }).context("Failed to parse container.xml")?;
...
fn build_toc_from_nav_document<R: Read + Seek>(
	...
	let nav_doc = XmlDocument::parse_with_options(&nav_content, ParsingOptions { allow_dtd: true, ..ParsingOptions::default() }).ok()?;
