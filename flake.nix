    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        cargoVendorDir = pkgs.rustPlatform.importCargoLock {
          lockFile = ./Cargo.lock;
          outputHashes = {
            "pdfium-0.1.1" = "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
          };
        };
        ...
        # wxWidgets source for wxdragon-sys (needs to be pre-downloaded for Nix sandbox)
        # wxdragon-sys 0.9.14 uses wxWidgets 3.3.2
        wxWidgetsSource = pkgs.fetchurl {
        ...
        # Wrapper script to put pdfium library on LD_LIBRARY_PATH
        pdfiumLib = "${pkgs.pdfium-binaries}/lib";

        # Extract version from Cargo.toml
        cargoToml = builtins.fromTOML (builtins.readFile ./Cargo.toml);
        paperbackVersion = cargoToml.package.version;
        ...
        # Shared build configuration for package and devShell
        commonNativeBuildInputs = with pkgs; [
          ...
          # wxdragon-sys CMake build needs full C++ toolchain
          gcc
        ];

        commonBuildInputs = with pkgs; [
          openssl # HTTPS support (ureq)
          gtk3
          wxwidgets_3_2
          webkitgtk_4_1 # WebView support
          pdfium-binaries # PDF rendering
          # Explicitly linked by wxdragon-sys
          libxkbcommon
          libxtst
          # Wayland support (required by wxWidgets 3.3.x)
          wayland
          wayland-scanner
          wayland-protocols
        ];

        # Environment variables needed for bindgen and build scripts
        commonEnv = {
          # Set LIBCLANG_PATH for bindgen
          LIBCLANG_PATH = "${pkgs.llvmPackages.libclang.lib}/lib";
          # Set BINDGEN_EXTRA_CLANG_ARGS to include libc headers
          BINDGEN_EXTRA_CLANG_ARGS = "-isystem ${pkgs.stdenv.cc.libc.dev}/include";
          # chmlib tarball for libchm build.rs
          CHMLIB_TARBALL = chmlibTarball;
          # wxWidgets source archive — extracted in preConfigure for nix build,
          # or let wxdragon-sys download it in devShell
          WXWIDGETS_ARCHIVE_PATH = wxWidgetsSource;
        };

        # Main Paperback package
        # Use regular stdenv (gcc) instead of clangStdenv for better CMake compatibility
        paperback = pkgs.stdenv.mkDerivation (
          commonEnv
          // {
            pname = "paperback";
            ...
            preConfigure = ''
                          # Extract wxWidgets source and set WXWIDGETS_DIR so build.rs skips downloading
                          mkdir -p wxWidgets-extracted
                          ${pkgs.unzip}/bin/unzip -qo $WXWIDGETS_ARCHIVE_PATH -d wxWidgets-extracted
              ...
              fn main() {
                  ...
                  println!("cargo:rustc-link-lib=static=chm");
                          ...
                          # wxdragon-sys 0.9.14 natively respects WXWIDGETS_DIR, no patching needed
            ...
            # Don't use CMake configure - we're using cargo directly
            ...
            installPhase = ''
              ...
              # Install binary and wrap with pdfium library path
      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell (
          ...
          // {
            ...
            # pdfium needs to be on LD_LIBRARY_PATH for runtime loading
