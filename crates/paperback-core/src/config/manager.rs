...
//! per-document lookups keyed by a stable content hash, sidecar `.paperback` file
//! import/export, and the generic `get_app_*`/`set_app_*` accessors host UIs use for
//! settings that don't warrant a dedicated field. Plus [`get_sorted_document_list`] and
//! [`compute_document_hash`], the two free functions built on top of it.
...
use super::{
	settings::{
		Bookmark, ConfigData, DocumentConfig, FindSettings, NavigationHistory, ReadabilityFont, StoredBookmark,
	},
	shortcuts::{ActionId, HotkeyConfig, KeyChord, ShortcutsConfig},
};
